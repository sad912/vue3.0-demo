...
* `watchEffect()` 函数的参数为一个回调函数，`watchEffect()` 会立即执行一遍回调函数，如果这时函数产生了副作用，Vue
  会自动追踪副作用的依赖关系，分析出响应式源数据，每当响应式源数据发生变化时，回调函数会再次执行。
* 当响应式数据发生改变的时候，可能会同时触发 Vue 组件的更新和侦听器函数，默认情况下，侦听器函数的执行会先于 Vue 组件更新，也就是说，此时的侦听器访问的 DOM 是旧的状态。
...
* Vue 有两种方式来注册组件：全局注册和局部注册。
* 全局注册
    * 通过 `app.component()` 函数进行全局注册。
    * 第一个参数为注册组件名，第二个参数为组件实现对象。
    * 第二个参数可以为单文件组件。
    * `.component()` 可以链式调用，注册多个组件。
    * 全局注册的组件可以在此应用的任意组件中使用。
    * 全局注册的组件无法被 tree-shaking。
    * 全局注册的组件会使依赖关系不明确。
* 局部注册
    * 选项式 API 组件的局部注册，通过 `components` 选项进行声明。
    * 组合式 API 组件的局部注册，当你在单文件组件中使用了 `<script setup>`，导入的组件可以在本地使用而无需注册；如果不在 `<script setup>` 中，你将需要使用 components 选项。

#### Props

* 组件需要显式声明 prop，这样 Vue 才知道外部传入的哪些是 prop，哪些是透传 attribute。
* 选项式 API 中，通过 `props` 选项来定义。
* 在单文件组件中使用 `<script setup>` 时，属性可以使用 `defineProps()` 宏来定义。
* 在没有使用 `<script setup>` 的组件中，属性可以使用 `props` 选项来定义，`setup()` 会将 `props` 作为第一个参数。
* `props` 的值可以为属性名字符串数组或规定属性类型的对象。
* 组件使用的时候，仅在组件上声明自定义属性而不赋值，会隐式赋值为 `true`。
* 组件使用的时候，自定义属性的值是静态非字符串类型的时候，应该使用 `v-bind`。
* 选项式 API 中，如果想将多个属性传入组件，可以在组件上双向绑定一个包含这些属性的响应式对象，类似解构赋值。
* 所有的组件属性都具有单项数据流的特点，自然而然的由父组件属性的改变触发子组件属性的更新，子组件的属性是只读的，不可修改。
* 如果想操作子组件的属性，且此属性不需要根据父组件传入的数据进行响应式变化，那么可以定义一个属于子组件的响应式数据，并将属性值赋给它。
* 如果想操作子组件的属性，且需要其根据父组件传入的数据进行响应式变化，可以添加一个计算属性用于转化。
* 选项式 API 中，可以通过 `this` 直接访问属性值，组合式 API 中，可以通过 `defineProps()` 函数的返回值来访问属性值。
* 子组件是可以更改数组或对象类型的属性的，这时的数据流是混乱的，应该避免这样的更改，子组件应该抛出一个事件通知父组件对属性值做出改变。
* `props` 的值为对象时，可以对属性做一些校验，包括类型、缺省值、`required` 状态，甚至可以编写缺省值函数和校验函数。
* `required` 默认值为 `true`，`default` 默认值为 `undefined`。
...
```
* 属性的的校验是在组件实例被创建之前，所以实例的属性（比如 `data`、`computed` 等）将在 `default` 或 `validator` 函数中不可用。
* 类型检查可以是原生的类型构造器，也可以是自定义的类和构造函数。
* 布尔类型的属性在组件中使用时，可以仅声明属性名，此时默认为 `true`，如果不声明，则默认为 `false`。

#### 组件事件

* 子组件通过 `$emit()` 触发对应组件使用时用 `@` 声明的事件。
* 可以通过给 `$emit()` 多余的参数实现对事件的传参。
* 选项式 API 中，组件要触发的事件可以显式地通过 `emits` 选项来声明，它的值可以为触发事件名的字符串组成的数组，可以为包含对触发参数进行校验的函数的对象。
* 使用 `<script setup>` 的单文件组件可以显式地通过 `defineEmits()` 来声明，参数可以为触发事件名的字符串组成的数组，可以为包含对触发参数进行校验的函数的对象。
* 如果你没有使用 `<script setup>`，则事件需要通过 `emits` 选项来定义，`emit` 函数作为第二参数被暴露在 `setup()` 的上下文对象上。
* 如果一个原生事件的名字（例如 `click`）被定义在 `emits` 选项中，则监听器只会监听组件触发的 `click` 事件而不会再响应原生的 `click` 事件。
* 为事件添加校验，那么事件可以被赋值为一个函数，接受的参数就是抛出事件时传入 `emit` 的内容，返回一个布尔值来表明事件是否合法。
* 组件使用时声明 `v-model`，相当于声明了一个名为 `modelValue` 的 `props` 和 一个名为 `update:modelValue` 的事件，子组件内部可以通过 `$emit("update:modelValue", newValue)` 触发具体修改双向绑定的属性的值。
* `modelValue` 为默认的 `v-model` 名，也可以指定一个参数来更改默认名，例如 `v-model:name`。
* 通过修改默认的 `v-model` 名，可以在组件中使用多个 `v-model`。
* 组件中使用 `v-model` 可以添加自定义修饰符，相当于声明了一个名 `modelModifers` 的 `props`，在子组件中其默认值为一个空对象，可以通过属性访问自定义修饰符的名字来实现修饰符的逻辑，值为布尔类型。
* 对于有参数的 `v-model` 的自定义修饰符，生成的 `props` 名为参数名 + `Modifers`。
...
* 深层组件在满足透传规则时也会发生属性透传，但透传的属性不会包含子组件上声明过的 props 或 事件。
* 如果你不想使组件自动地继承属性，你可以在选项式 API 或不使用 `<script setup>` 单文件组件中设置 `inheritAttrs` 选项为 `false`，或者在使用 
`<script setup>` 的单文件组件中额外的使用一个 `<script>` 块来书写这个选项声明。
* 常用的禁用继承场景是将父组件属性继承到非子组件的根元素上，这时的透传属性可以使用 `$attrs` 访问，这个 `$attrs` 对象包含了除组件的 `props` 和
`emits` 属性外的所有其他属性，例如 class，style，v-on 监听器等等。
* 没有参数的 `v-bind` 会将一个对象的所有属性都作为 attribute 应用到目标元素上,所以可以将 `$attrs` 通过双向绑定到子元素上实现非根元素的组件属性继承。
* 有几点需要注意： 
  * 和 props 有所不同，透传 attributes 在 JavaScript 中保留了它们原始的大小写，所以像 `foo-bar` 这样的一个属性需要通过 `$attrs['foo-bar']` 来访问。 
  * 像 `@click` 这样的一个 v-on 事件监听器将在此对象下被暴露为一个函数 `$attrs.onClick`。
* 有着多个根节点的组件没有自动 attribute 透传行为。
* 在选项式 API 中，可以通过 `this.$attrs` 访问所有透传 attribute，在组合式 API中，使用了 `<script setup>` 的单文件组件中的 `useAttrs()`
函数的返回值为组件的透传属性对象，没使用 `<script setup>` 的单文件组件中 `setup` 函数的第二个参数具有 `attrs` 方法，可以通过此方法访问所有透传属性。

#### 插槽

* `props` 为数据在父子组件在数据方面提供了通信方式，插槽为父子组件在模板片段方面提供了通信方式。
* 通过在子组件内使用 `<slot></slot>` 声明，并在使用子组件时，在内部插入模板片段实现插槽。
* 插槽内容可以访问到父组件的数据作用域，但无法访问子组件的数据。
* 可以在 `<slot>` 标签内声明默认的插槽内容。
* 匿名插槽默认将使用组件时的内部模板片段作为插槽内容，通过添加 `name` 属性的具名插槽将使用组件时的内部声明 `v-slot:{name}` 属性 `template`
元素作为插槽内容。
* `v-slot:` 可以简写为 `#`。
* 动态指令参数在 `v-slot` 上也是有效的，可以通过 `[]` 来实现动态插槽名。
* 子组件上的插槽上声明属性，父组件可以在 `v-slot` 属性上拿到子组件声明过的属性（除保留的 `name` 属性外）。
* 一些组件可能只包括了逻辑而不需要自己渲染内容，视图输出通过作用域插槽全权交给了消费者组件。我们将这种类型的组件称为无渲染组件。

#### 依赖注入

* 为了避免 ”prop drilling“，Vue 提供了 `provide` 和 `inject`，用于为父组件和其后代组件提供通信方式。
* 在选项式 API 中，通过 `provide` 选项实现属性声明，值可以为包含属性的对象，也可以为返回包含属性的对象的函数（这样可以通过 `this` 访问响应式数据）。
* 在组合式 API 中，如果使用了 `<script setup>`，需要调用 `provide()` 函数实现属性的注册；如果不使用 `<script setup>`，可以在 `setup()` 函数中同步调用
`provide` 函数。函数第一个参数为属性名字符串或 `Symbol`，第二个参数为属性值。
* 应用层也提供了 `provide` 方法，可以给整个应用注入依赖属性。
* 在选项式 API 中，后代组件可以通过 `inject` 选项，为组件声明注入 `provide` 提供的属性，值可以为属性值字符串组成的数组，然后组件可以通过 `this`
来访问该属性，注入会在组件自己的状态前被解析，所有可以在 `data()` 中访问注入的属性。
* 在组合式 API 中，如果使用了 `<script setup>`，需要使用 `inject()` 函数；如果不使用 `<script setup>`，需要在 `setup()` 函数中同步调用
`inject()` 函数。参数为属性名，返回值为属性值，如果属性值是`ref`，则不会被解包，这可以保持数据的响应式。
* 在选项式 API 中，由于 `inject` 选项注入的属性会暴露在 `this` 上，会发生重名的问题，可以通过注入别名来解决这个问题，通过声明一个别名的属性，为之赋值一个具有
`from` 属性的对象来实现，`from` 的值为原注入属性名。
* 在选项式 API 中，可以设置注入属性的默认值，此时 `inject` 选项的值必须为对象类型，对象的键为属性名，值为包含 `default` 属性的对象，对于普通数据类型的注入属性，`default`
属性的值为默认值，对于复杂数据类型的注入属性，`default` 的值为工厂函数。
* 在组合式 API 中，可以通过设置 `inject` 函数的第二个参数来实现注入属性的默认值，对于普通数据类型，第二个参数为默认值，对于复杂数据类型，第二个参数为一个工厂函数。
* 在选项式 API 中，可以通过设置 `provide` 选项的值为返回包含属性对象的函数，同时将计算属性函数作为属性值来实现依赖的响应式变化。
* 在组合式 API 中，通过 `provide()` 函数注册一个 `ref` 数据就可以实现响应式，但 Vue 推荐对属性的注册和变更都在 `provide()` 内部，通过声明修改属性的函数来实现，如果你想确保从
`provide` 传过来的数据不能被 `injector` 的组件更改，你可以使用 `readonly()` 来包装提供的值。
* 为避免潜在的冲突，建议最好使用 Symbol 来作为注入名。
...
* ES 模块动态导入也会返回一个 `Promise`，所以多数情况下我们会将它和 `defineAsyncComponent` 搭配使用，导入一个单文件组件。
